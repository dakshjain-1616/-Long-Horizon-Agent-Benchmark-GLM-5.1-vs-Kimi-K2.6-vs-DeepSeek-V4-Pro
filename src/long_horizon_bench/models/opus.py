"""Claude Opus 4.7 model client (via OpenRouter, OpenAI-compatible API)."""
...
class OpusClient(BaseModelClient):
    """Client for Claude Opus 4.7 via OpenRouter."""

    # Verified on OpenRouter, April 2026: $5/M in, $25/M out.
    ...
    async def chat(self, messages: list[Message], tools: list[dict[str, Any]] | None = None, tool_choice: str | None = None) -> ChatResponse:
        ...
        op_messages = self._convert_messages(messages)
        payload: dict[str, Any] = {
            "model": self.config.model or "anthropic/claude-opus-4-7",
